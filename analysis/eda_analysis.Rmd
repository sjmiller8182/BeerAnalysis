...
```{r maps, echo=TRUE}
source("background_code/abb2name.R")
missing.by.state <- merged_data %>% 
  group_by(State) %>% 
  summarise(ABV_missing=prop_miss(ABV), IBU_missing=prop_miss(IBU))
missing.by.state$region <- abb2state(missing.by.state$State)
...
ggplot(map.df, aes(x=long,y=lat,group=group))+
  geom_polygon(aes(fill=IBU_missing))+
  geom_path()+ 
...
ggplot(map.df, aes(x=long,y=lat,group=group))+
  geom_polygon(aes(fill=ABV_missing))+
  geom_path()+ 
  scale_fill_gradientn(colours=rev(heat.colors(10)),na.value="grey90")+
  coord_map()+
...
summary_df$region <- abb2state(summary_df$State)
states <- map_data("state")
